...
namespace DAL
{
    ...
    public class SQLiteHelper
    {
        ...
        {//"Data Source=Test.db3;Pooling=true;FailIfMissing=false";  
            //C:\Projects\windows\XL-Laboratory-Test\Src\XL-Lab\XL-Lab\bin\x86\Debug
            get
            {
                string dir = AppDomain.CurrentDomain.BaseDirectory;
                return $"Data Source={dir}xllabserverdb.db;Pooling=true;FailIfMissing=false";
                // return @"Data Source=C:\Projects\windows\XL-Laboratory-Test\Src\XL-Lab\XL-Lab\bin\x86\Debug\xllabdb.db;Pooling=true;FailIfMissing=false";
            }
        }
        /// <summary>  
        /// 连接对象  
        ...
        public static DataSet ExecuteDataSet(string commandText, params object[] paramList)
        {
            ...
            if (paramList != null)
            {
                AttachParametersEx(cmd, commandText, paramList);
            }
        ...
        public static DataSet ExecuteDataSet(SQLiteTransaction transaction, string commandText, object[] commandParameters)
        {
            ...
            IDbCommand cmd = transaction.Connection.CreateCommand();
            cmd.CommandText = commandText;
            AttachParametersEx((SQLiteCommand)cmd, cmd.CommandText, commandParameters);
            if (transaction.Connection.State == ConnectionState.Closed)
                transaction.Connection.Open();
            DataSet ds = ExecuteDataSet((SQLiteCommand)cmd);
            return ds;
        ...
        public static int ExecuteNonQuery(SQLiteTransaction transaction, string commandText, params  object[] paramList)
        {
            ...
            {
                cmd.CommandText = commandText;
                AttachParametersEx((SQLiteCommand)cmd, cmd.CommandText, paramList);
                if (transaction.Connection.State == ConnectionState.Closed)
                    transaction.Connection.Open();
                int result = cmd.ExecuteNonQuery();
                return result;
        ...
        #region AttachParametersEx(SQLiteCommand,commandText,object[] paramList)
